...
(function (global) {
  ...
  const PRESETS = {
    robe: {
      'robe-1': { name: 'Design Robe Camel — Parements Bordeaux (Photo de référence)',
                  src: 'img/designs/robe/robe-1-1.webp',
                  apply: { sleeve: 'modele-1', finish: 'mat', fabricColor: 'camel' } },
  ...
  async function applyPresetModel(id) {
    ...
    /* La photo arrive après le premier rendu de l'écran : sans ce
       rafraîchissement, elle est bien dans la commande mais la cliente
       ne la voit pas dans sa liste de fichiers. */
    if (document.getElementById('czFiles')) renderFiles();
    else renderRail();
